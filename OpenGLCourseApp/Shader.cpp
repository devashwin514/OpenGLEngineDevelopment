...
Shader::Shader()
{
	shaderID = uniformModel = uniformProjection = uniformView = pointLightCount = spotLightCount = 0;
}
...
GLfloat Shader::GetAmbientIntensityLocation() {
	return uniformDirectionalLight.uniformAmbientIntensity;
}

GLfloat Shader::GetAmbientColorLocation() {
...
void Shader::SetPointLights(PointLight * pLight, unsigned int lightCount)
{
	...
	for (size_t i = 0; i < lightCount; i++) {
		pLight[i].UseLight(uniformPointLight[i].uniformAmbientIntensity, uniformPointLight[i].uniformColor, 
						...
						uniformPointLight[i].uniformLinear, uniformPointLight[i].uniformExp);
	}
}
...
void Shader::SetSpotLights(SpotLight * sLight, unsigned int lightCount)
{
	...
	for (size_t i = 0; i < lightCount; i++) {
		sLight[i].UseLight(uniformSpotLight[i].uniformAmbientIntensity, uniformSpotLight[i].uniformColor,
			...
			uniformSpotLight[i].uniformConstant, uniformSpotLight[i].uniformLinear, uniformSpotLight[i].uniformExp, 
...
void Shader::SetDirShadowMap(GLuint texUnit)
{
	glUniform1i(uniformDirShadowMap, texUnit);
}

void Shader::SetDirLightTransform(glm::mat4* ltransform)
{
	glUniformMatrix4fv(uniformDirLightTransform, 1, GL_FALSE, glm::value_ptr(*ltransform));
}

void Shader::SetLightMatrices(std::vector<glm::mat4> lightMat)
{
	for (size_t i = 0; i < 6; i++) {
		glUniformMatrix4fv(uniformLightMatrices[i], 1, GL_FALSE, glm::value_ptr(lightMat[i]));
	}
}
...
//void Shader::CreateFromFiles(const char * vertexLocation, const char * geomLocation, const char * fragmentLocation)
//{
//	std::string vertexString = ReadFile(vertexLocation);
//	std::string fragmentString = ReadFile(fragmentLocation);
...
std::string Shader::ReadFile(const char* fileLocation) {
	...
	if (!fileStream.is_open()) {
		printf("Cannot open file: ", fileLocation);
		return "";
	}
...
void Shader::CompileProgram()
{
	...
	uniformDirectionalLight.uniformColor = glGetUniformLocation(shaderID, "dirLight.base.colour");
	...
	uniformSpecularIntensity = glGetUniformLocation(shaderID, "matrl.specularIntensity");
	uniformShininess = glGetUniformLocation(shaderID, "matrl.shininess");
	uniformEyePostion = glGetUniformLocation(shaderID, "eyePosition");
	uniformTexture = glGetUniformLocation(shaderID, "theTexture");
	uniformDirLightTransform = glGetUniformLocation(shaderID, "dirLightSpaceTransform");
	...
	for (size_t i = 0; i < MAX_POINT_LIGHTS; i++) {
		...
		uniformPointLight[i].uniformExp = glGetUniformLocation(shaderID, locBuff);
	}
	...
	for (size_t i = 0; i < MAX_SPOT_LIGHTS; i++) {
		...
		uniformSpotLight[i].uniformExp = glGetUniformLocation(shaderID, locBuff);
